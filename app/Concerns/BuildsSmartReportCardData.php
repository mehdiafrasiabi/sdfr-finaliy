...
/**
 * منطق مشترک ساخت دیتای «کارنامهٔ هوشمند».
 *
 * این تِرِیت بین کامپوننت دانش‌آموز (Client\Profile\SmartReportCardShow) و
 * کامپوننت مشاور (Admin\Student\SmartReportCard\View) به اشتراک گذاشته می‌شود تا
 * هر دو سمت دقیقاً یک دیتای یکسان (هیچ اطلاعاتی از قلم نیفتد) را تولید کنند.
 *
 * تمام متدها بر اساس $studentId و بازهٔ زمانی کار می‌کنند و به کاربر لاگین‌شده
 * وابسته نیستند، بنابراین مشاور می‌تواند کارنامهٔ هر دانش‌آموز را بسازد.
 */
trait BuildsSmartReportCardData
{
    /**
     * تمام آرایه‌های مورد نیاز ویوِ کارنامه را برای یک کارت ماهانه می‌سازد.
     * این متد بین سمت دانش‌آموز و سمت مشاور مشترک است تا خروجی کاملاً یکسان باشد.
     *
     * @param SmartReportCard $card        کارت ماه (می‌تواند ذخیره‌نشده/transient باشد)
     * @param array           $availableGrades پایه‌های قابل نمایش (مثلاً ['10','11','12'])
     */
    private function buildReportCardPayload(SmartReportCard $card, array $availableGrades): array
    {
        $studentId = $card->student_id;

        $currentStart = $card->start_date->copy()->startOfDay();
        $currentEnd = $card->end_date->copy()->endOfDay();

        $prevMonth = $card->jalali_month - 1;
        $prevYear = $card->jalali_year;
        if ($prevMonth < 1) {
            $prevMonth = 12;
            $prevYear -= 1;
        }
        ...
        return [
            ...
            'analysis' => $analysis,
            'currentLabel' => $card->month_name . ' ' . $card->jalali_year,
