<div>
        ...
        <div class="result-container min-h-screen bg-gradient-to-br
         from-slate-50 via-blue-50 to-slate-100 dark:from-slate-950 dark:via-blue-950 dark:to-slate-950 py-8">
                ...
                <div class="mb-8">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <h1 class="font-black text-3xl md:text-4xl text-transparent bg-clip-text bg-gradient-to-l from-blue-600 to-blue-400">
                            <span class="text-white"> آزمون: {{ $exam->title }}</span>
                        </h1>
                    </div>
                    <p class="text-lg font-semibold text-slate-600 dark:text-slate-300 mb-6">دانش‌آموز: {{ optional($student->personalInformation)->name }}</p>
...
<br>
                    @if($correctCount === $exam->number_of_questions && $unansweredCount === 0)
                        <div class="performance-message rounded-2xl p-6 mb-8 text-center">
                            ...
                            <h2 class="text-2xl font-black text-white mb-2">تبریک! عملکرد فوق‌العاده‌ای ثبت شده است.</h2>
                            <p class="text-green-700 dark:text-green-400">این دانش‌آموز به تمام سوالات پاسخ صحیح داده است.</p>
                        </div>
                    @elseif($correctCount > $incorrectCount)
                        <div class="performance-message rounded-2xl p-6 mb-8">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 class="font-bold text-xl text-green-500 dark:text-green-300">عملکرد خوبی ثبت شده است!</h3>
                                    <p class="text-green-500 dark:text-green-400">تعداد پاسخ‌های صحیح بیشتر از پاسخ‌های نادرست است.</p>
                                </div>
                            </div>
                        </div>
                    @endif
<br>
                    <div class="grid grid-cols-1 2xl:grid-cols-3 gap-[25px] mb-[25px] rounded-md">
                        <div class="stats-box glass-bg rounded-2xl p-6 border border-green-200 dark:border-green-900 text-center">
                            ...
                            <p class="text-3xl font-black text-success-600 dark:text-green-400">{{ $correctCount }}</p>
                        </div>
                        <div class="stats-box glass-bg rounded-2xl p-6 border border-red-200 dark:border-red-900 text-center">
                            ...
                            <p class="text-3xl font-black text-danger-500 dark:text-red-400">{{ $incorrectCount }}</p>
                        </div>
                        <div class="stats-box glass-bg rounded-2xl p-6 border border-amber-200 dark:border-amber-900 text-center">
                            ...
                            <p class="text-3xl font-black text-warning-500 dark:text-amber-400">{{ $unansweredCount }}</p>
                        </div>
                    </div>
                </div>
                    <br>
                @if($examPdf || $solutionPdf)
                    <div class="grid grid-cols-1 2xl:grid-cols-2 gap-[25px] mb-[25px] rounded-md">
                        @if($examPdf)
                ...
                <div class="glass-bg rounded-2xl border border-blue-200 dark:border-blue-900 p-6 mb-5">
                    ...
                    @if($analysisStatus === 'sent' && !$uploadSuccess)
                        <div class="text-center py-8">
                            <div class="success-checkmark text-6xl mb-4">✅</div>
                            <h4 class="font-bold text-xl text-green-500 dark:text-green-400 mb-2">تحلیل آپلود شده است</h4>
                            <p class="text-slate-800 dark:text-slate-400 mb-6">تصاویر تحلیل دانش‌آموز در زیر قابل مشاهده است.</p>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        @foreach($urls as $idx => $img)
                                            <button type="button"
                                                ...
                                                <img src="{{ $img }}" alt="analysis-{{ $idx }}" class="w-full h-20 object-cover">
                                            </button>
                                        @endforeach
                                    </div>
                    ...
                    @elseif($analysisStatus === 'expired')
                        <div class="text-center py-8 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900">
                            ...
                            <h4 class="font-bold text-xl text-red-500 dark:text-red-400 mb-2">مهلت تمام شده است</h4>
                            <p class="text-slate-800 dark:text-slate-400">پس از ۴۸ ساعت از تکمیل آزمون امکان آپلود تحلیل وجود ندارد.</p>
                        </div>
                    @else
                        <form wire:submit="uploadAnalysis">
                            <div class="space-y-4">
                                ...
                                <button type="submit"
                                        ...
                                        class="w-full py-3 px-4 rounded-xl font-bold text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-md hover:shadow-lg">
                ...
                <div class="glass-bg rounded-2xl p-4 border border-slate-200 dark:border-slate-700 mb-5">
                    <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                        ...
                        <span class="text-rose-500">تحلیل دانش‌آموزان را در بازه ۴۸ ساعته دریافت و بررسی کنید تا بازخورد دقیقی ارائه شود.</span>
                    </p>
                </div>
<br>
                <div class="glass-bg rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                    ...
                    <div class="grid grid-cols-1 2xl:grid-cols-3 gap-[25px] mb-[25px] rounded-md">
                        @foreach($this->allQuestions() as $q)
                            <div class="question-item rounded-2xl dark:border-slate-700 overflow-hidden mb-2">
                                <div class="flex">
                                ...
                                @endif"></div>
                                    ...
                                    <div class="flex-1 p-4">
                                        <div class="flex items-start justify-between gap-4 mb-3">
                                            ...
                                            <div>
                                                @if($q['status'] === 'correct')
                                                    <span class=" px-[8.3px] py-[1px] badge-correct rounded-full inline-block text-white  border border-success-300
                                                     bg-success-100 dark:bg-[#15203c] dark:border-[#172036]">
                                                        پاسخ صحیح
                                                    </span>
                                                @elseif($q['status'] === 'incorrect')
                                                    <span class=" px-[8.3px] py-[1px] inline-block text-white  badge-incorrect items-center gap-1 rounded-full px-3 py-1 text-white text-xs font-bold shadow-md">
                                                        پاسخ نادرست
                                                    </span>
                                                @else
                                                    <span class="px-[8.3px] py-[1px] inline-block text-white badge-unanswered inline-flex items-center gap-1 rounded-full px-3 py-1 text-white text-xs font-bold shadow-md">
                                                        بی‌پاسخ
                                                    </span>
                                                @endif
                                            </div>
                                    ...
                                    @endif">
                                            <div class="space-y-3">
                                                <div>
                                                    <p class="text-xs font-semibold text-slate-800 dark:text-slate-800 mb-1">پاسخ دانش‌آموز:</p>
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-xl font-bold text-success-600 dark:text-success-600">
