<div class="relative min-h-screen overflow-hidden bg-background text-foreground" dir="rtl" x-data="forgotPasswordForm()">
    ...
    @push('link')
        <style>
            ...
            .train-border { position: relative; border-radius: 1.5rem; --bw: 2px; --speed: 3s; }
            .train-border::before {
                content: '';
            ...
            .step-dot.active { width: 28px; background: hsl(var(--primary)); }
            .step-dot.completed { background: hsl(var(--primary) / 0.5); }
        ...
        <div class="w-full max-w-md">
            ...
            <div class="flex flex-col items-center mb-6">
                ...
                @if($step < 4)
                    <div class="flex items-center gap-2 mt-4">
                        @for($i = 1; $i <= 3; $i++)
                            <span class="step-dot {{ $step === $i ? 'active' : ($step > $i ? 'completed' : '') }}"></span>
                        @endfor
                ...
                <div class="glass-card rounded-3xl p-6 space-y-5">
                        ...
                        <form wire:submit.prevent="verifyCode" autocomplete="off" class="space-y-4">
                            <div class="rounded-xl p-3.5 bg-primary/5 border border-primary/20">
                                <p class="text-xs text-center text-foreground/80">
                                    کد ارسال‌شده به <span class="font-bold text-primary" dir="ltr">{{ $mobile }}</span> را وارد کنید
                                </p>
                            ...
                            <div class="flex items-center justify-center">
                                ...
                                <button type="button" x-show="countdown === 0" x-cloak wire:click="resendCode" @click="startCountdown(90)"
                        ...
                        <form wire:submit.prevent="resetPassword" autocomplete="off" class="space-y-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="mt-3 p-3 rounded-xl bg-secondary/40 border border-border space-y-2">
                                    ...
                                    @foreach([
                                        ...
                                        <div class="flex items-center gap-2 text-xs transition-colors {{ $ok ? 'text-emerald-500' : 'text-muted' }}">
                            ...
                            <div class="relative">
                                ...
                                <div class="password-wrapper">
                                    ...
                                    <button type="button" class="eye-btn" @click="showPasswordConfirmation = !showPasswordConfirmation" tabindex="-1">
                                        <svg x-show="!showPasswordConfirmation" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                                        </svg>
                                        <svg x-show="showPasswordConfirmation" x-cloak class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/>
                                        </svg>
                                    </button>
                                </div>
                                @error('passwordConfirmation')<div class="text-xs text-rose-500 mt-1.5 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                    {{ $message }}
                                </div>@enderror
                            </div>
                    ...
                    @if($step === 4)
                        <div class="text-center space-y-4">
                            <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-emerald-500/15 border border-emerald-500/30 anim-pop">
                                <svg class="h-8 w-8 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
                                </svg>
                            </div>
                            <h2 class="text-xl font-black text-foreground">رمز عبور با موفقیت تغییر کرد</h2>
                            <p class="text-sm text-muted leading-7">اکنون می‌توانید با رمز عبور جدید وارد حساب کاربری خود شوید.</p>

                            <button type="button" wire:click="loginAndRedirect" wire:loading.attr="disabled" wire:target="loginAndRedirect"
                                    class="btn-press w-full h-12 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                                <span wire:loading.remove wire:target="loginAndRedirect" class="flex items-center gap-2">
                                    ورود به حساب کاربری
            ...
            <div class="mt-4 text-center">
                <p class="text-[11px] text-muted leading-6">
                    ...
                    <a wire:navigate href="{{ route('client.terms') }}" class="text-foreground hover:text-primary hover:underline font-semibold">شرایط</a>
                    و
                    <a wire:navigate href="{{ route('client.terms') }}" class="text-foreground hover:text-primary hover:underline font-semibold">قوانین حریم خصوصی</a>
                    است.
                </p>
            </div>
        </div>
