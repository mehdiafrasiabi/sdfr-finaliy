<div>
    @push('link')
        ...
        <style>
            ...
            .btn-press:active:not(:disabled), .btn-press.pressed {
            ...
            .btn-press-secondary:hover:not(:disabled) {
                transform: translateY(-1px);
            }
            ...
            .skeleton {
                background: linear-gradient(90deg, hsl(var(--secondary)) 25%, hsl(var(--border) / 0.5) 50%, hsl(var(--secondary)) 75%);
    ...
    @php
        ...
        if (! $examPlanMode && empty($fieldOptions)) {
    ...
    <div class="relative min-h-screen overflow-hidden bg-background text-foreground" dir="rtl" x-data="onboardingFlow()"
         :class="$wire.currentStep === 5 ? 'otp-stage-mode' : ''">
        ...
        <div
            class="absolute bottom-20 -left-20 w-80 h-80 bg-primary/10 rounded-full blur-3xl float-orb pointer-events-none"
        ...
        <div x-data="{ openAv: false }"
            ...
            <div
                class="relative w-full max-w-md glass-card rounded-t-[2rem] sm:rounded-3xl px-5 pt-4 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] sm:p-6"
                x-show="openAv"
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" @click="openAv = false"
                            class="w-8 h-8 rounded-lg bg-secondary/60 flex items-center justify-center hover:bg-secondary">
                ...
                <div class="grid grid-cols-3 gap-4" x-show="gender === 'male'">
                    ...
                    @empty
                        <div
                            class="col-span-3 rounded-2xl border border-dashed border-border px-4 py-6 text-center text-xs text-muted">
        ...
        <div class="md:hidden relative z-10 min-h-[100dvh] flex flex-col">
                    ...
                    <div class="flex-1 flex items-center justify-center gap-2">
                        @for ($i = 2; $i <= $totalSteps; $i++)
                            <span class="progress-dot"
                                  :class="{ 'active': $wire.currentStep === {{ $i }}, 'completed': $wire.currentStep > {{ $i }} }"></span>
                        @endfor
                    </div>
                    <div class="text-xs font-mono text-muted"><span x-text="$wire.currentStep"></span>/{{ $totalSteps }}
                    </div>
                </div>
            </header>
            ...
            @if ($generalError)
                <div
                    class="mx-4 mb-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-500 px-4 py-3 text-sm">{{ $generalError }}</div>
            @endif

            <main class="flex-1 flex items-stretch justify-center px-4 py-4" @touchstart="handleTouchStart($event)"
                ...
                <div class="w-full relative">
                    ...
                    <div x-show="busy" x-cloak
                        ...
                        <svg class="w-12 h-12 animate-spin text-primary" viewBox="0 0 24 24" fill="none">
                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" stroke-dasharray="48"
                                    stroke-linecap="round" opacity="0.4"/>
                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"
                                    stroke-dasharray="14 60" stroke-linecap="round"/>
                        </svg>
                            ...
                            <div class="train-border">
                                <div class="glass-card rounded-3xl p-6">
                                    ...
                                    <form @submit.prevent="goNext()" autocomplete="on" class="space-y-4">
                                        ...
                                        <div class="relative">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">نام
                                                خانوادگی</label>
                                        ...
                                        <div class="relative" data-tour="codeMell">
                                            ...
                                            <input wire:model.blur="codeMell" type="tel" maxlength="10"
                                                   placeholder="۱۰ رقم" inputmode="numeric" dir="ltr" autocomplete="off"
                            ...
                            <div class="train-border">
                                <div class="glass-card rounded-3xl p-6">
                                    <div class="flex items-center gap-3 mb-6">
                                        ...
                                        <div><h2 class="font-black text-lg">والدین و پایه</h2>
                                            <p class="text-[11px] text-muted">برای گزارش‌گیری و ارتباط</p></div>
                                    </div>

                                    <form @submit.prevent="goNext()" class="space-y-4">
                                        <div class="relative">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">شماره
                                                پدر</label>
                                            <input wire:model.blur="fatherMobile" type="tel" maxlength="11"
                                                   placeholder="09..." dir="ltr" inputmode="numeric" autocomplete="off"
                                        ...
                                        <div class="relative">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">شماره
                                                مادر</label>
                                            <input wire:model.blur="motherMobile" type="tel" maxlength="11"
                                                   placeholder="09..." dir="ltr" inputmode="numeric" autocomplete="off"
                                        ...
                                        @if($grade === 'graduate')
                                            <div
                                                class="flex items-start gap-2 rounded-xl bg-primary/5 border border-primary/20 px-3.5 py-3">
                                                <svg class="w-4 h-4 text-primary shrink-0 mt-0.5" viewBox="0 0 24 24"
                                                ...
                                                <p class="text-[11px] text-muted leading-5">چون فارغ‌التحصیل هستی،
                                                    برنامه‌ات بدون نیاز به برنامه‌ی کلاسی مدرسه طراحی می‌شود.</p>
                                            </div>
                                        @elseif(! $examPlanMode)
                                            <div class="relative">
                            ...
                            <div class="train-border">
                                <div class="glass-card rounded-3xl p-6">
                                    <div class="flex items-center gap-3 mb-6">
                                        ...
                                        <div><h2 class="font-black text-lg">حساب کاربری</h2>
                                            <p class="text-[11px] text-muted">شماره و رمز ورود به سامانه</p></div>
                                    </div>

                                    <form @submit.prevent="goNext()" autocomplete="on" class="space-y-4">
                                        <div class="relative">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">شماره موبایل
                                                (برای ورود)</label>
                                            <input wire:model.blur="mobile" type="tel" maxlength="11"
                                                   placeholder="09..." dir="ltr" inputmode="numeric"
                                        ...
                                        <div class="relative" x-data="{ showPw: false }">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">رمز
                                                عبور</label>
                                            <div class="password-wrapper">
                                                <input wire:model.live.debounce.300ms="password"
                                                       :type="showPw ? 'text' : 'password'" dir="rtl"
                                                ...
                                                <button type="button" class="eye-btn" @click="showPw = !showPw"
                                                        tabindex="-1">
                                                    <svg x-show="!showPw" class="w-4 h-4" fill="none"
                                        ...
                                        <div class="relative" x-data="{ showPwc: false }">
                                            <label class="block text-xs font-semibold mb-1.5 text-muted">تکرار
                                                رمز</label>
                                            <div class="password-wrapper">
                                                <input wire:model.blur="passwordConf"
                                                       :type="showPwc ? 'text' : 'password'" dir="rtl"
                                                       autocomplete="new-password"
                                                       class="glass-input w-full rounded-xl px-4 py-2.5 text-sm @error('passwordConf') border-rose-500/60 shake @enderror">
                                                <button type="button" class="eye-btn" @click="showPwc = !showPwc"
                                                        tabindex="-1">
                                                    <svg x-show="!showPwc" class="w-4 h-4" fill="none"
                                                         viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                                                         stroke-linecap="round" stroke-linejoin="round">
                                                        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                                        <circle cx="12" cy="12" r="3"/>
                                                    </svg>
                                                    <svg x-show="showPwc" x-cloak class="w-4 h-4" fill="none"
                                                         viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                                                         stroke-linecap="round" stroke-linejoin="round">
                                                        <path
                                                            d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/>
                                    ...
                                    <div class="otp-panel rounded-[2rem] px-6 py-10 text-center">
                                        ...
                                        <form @submit.prevent="submitOtp()" autocomplete="off"
                                            ...
                                            <div
                                                ...
                                                <svg class="w-4 h-4 mt-0.5 flex-none text-blue-200/80" fill="none"
                                                     viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                                    <circle cx="12" cy="12" r="9"></circle>
                                                    <path d="M12 8h.01"></path>
                                                    <path d="M11 12h1v4h1"></path>
                                                </svg>
                                                <p>در صورتی که کد تأیید را دریافت نکردید، بخش اسپم پیامک‌های تلفن همراه
                                                    خود را بررسی کنید.</p>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <button type="button" wire:click="previous"
                                                        class="btn-press-secondary h-14 shrink-0 rounded-2xl px-5 text-sm font-black">
                                                    برگشت
                                                </button>
                                            </div>
                                        </form>
                        ...
                        <section x-show="$wire.currentStep === 6"
                            ...
                            <div class="train-border">
                                <div class="glass-card rounded-3xl p-6 text-center">
                                    ...
                                    <p class="text-sm text-muted leading-7">شرط ورود به خانواده SDFR آنالیز فردی
                                        هست!<br>الان خودکار به آزمون Mindet منتقل میشی.</p>
                                    <div class="mt-5 flex items-center justify-center gap-2 text-primary">
                                        <svg class="w-5 h-5 animate-spin" viewBox="0 0 24 24" fill="none">
                                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"
                                                    stroke-dasharray="48" stroke-linecap="round" opacity="0.35"/>
                                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"
                                                    stroke-dasharray="14 60" stroke-linecap="round"/>
                                        </svg>
            ...
            <footer
                class="sticky bottom-0 px-4 pb-4 pt-2 bg-gradient-to-t from-background via-background/95 to-transparent"
                x-show="$wire.currentStep >= 2 && $wire.currentStep <= 4">
                <div class="flex items-center gap-3">
                    <button type="button" @click="goPrev()" @mousedown="pressBtn($el)" x-show="$wire.currentStep > 2"
                            :disabled="busy"
                    ...
                    <button type="button" @click="goNext()" @mousedown="pressBtn($el)" :disabled="busy"
                        ...
                        <span x-show="!busy" class="flex items-center gap-2">
                            ...
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                 stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5"/><path
                                    d="m12 19-7-7 7-7"/></svg>
                        </span>
                        <span x-show="busy" class="flex items-center gap-2">
                            <svg class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12"
                                                                                                      r="10"
                                                                                                      stroke="currentColor"
                                                                                                      stroke-width="3"
                                                                                                      stroke-dasharray="32"
        ...
        <div class="hidden md:block relative z-10 min-h-screen">
            ...
            @if ($generalError)
                <div class="max-w-7xl mx-auto px-8 mt-4">
                    <div
                        class="rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-500 px-4 py-3 text-sm">{{ $generalError }}</div>
                </div>
            @endif
                            ...
                            <div class="otp-panel rounded-[2.25rem] px-10 py-16 text-center">
                                ...
                                <form @submit.prevent="submitOtp()" autocomplete="off"
                                    ...
                                    <div
                                        ...
                                        <svg class="w-5 h-5 mt-0.5 flex-none text-blue-200/80" fill="none"
                                             viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                            <circle cx="12" cy="12" r="9"></circle>
                                            <path d="M12 8h.01"></path>
                                            <path d="M11 12h1v4h1"></path>
                                        </svg>
                                        <p>در صورتی که کد تأیید را دریافت نکردید، بخش اسپم پیامک‌های تلفن همراه خود را
                                            بررسی کنید.</p>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <button type="button" wire:click="previous"
                                                class="btn-press-secondary h-14 shrink-0 rounded-2xl px-6 text-lg font-black">
                                            برگشت
                                        </button>
                                    </div>
                                </form>
                ...
                <section x-show="$wire.currentStep === 6"
                    ...
                    <div class="train-border">
                        <div class="glass-card rounded-3xl p-10 text-center">
                            ...
                            <p class="text-muted leading-8">به جمعِ ستارگانِ SDFR خوش اومدی.<br>همین الان خودکار به
                                آزمون شخصیت‌شناسی منتقل می‌شی.</p>
            ...
            <div x-show="$wire.currentStep < 5" class="max-w-7xl mx-auto px-8 pt-8 pb-12">
                <div class="grid grid-cols-12 gap-8 items-start">
                    ...
                    <div class="col-span-5 sticky top-8 space-y-6">
                        <p class="text-muted leading-8 text-sm">پلتفرم هوشمند پایش مطالعه و مشاوره‌ی تخصصی برای
                            دانش‌آموزان جدی. با تکمیل فرم روبه‌رو، حساب کاربری شما ساخته می‌شه و وارد یک هفته‌ی آزمایشی
                            رایگان می‌شید.</p>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($features as $f)
                                <div
                                    class="flex items-start gap-2.5 p-3 rounded-xl bg-secondary/50 border border-border">
                                    <span
                                        class="flex items-center justify-center w-8 h-8 rounded-lg bg-primary/10 text-primary border border-primary/20 shrink-0">
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    ...
                    <div class="col-span-7">
                        <div class="train-border">
                            <form @submit.prevent="submitDesktopForm()" autocomplete="on"
                                  class="glass-card rounded-3xl p-8 space-y-7">
                                ...
                                <div class="flex items-center gap-3 pb-5 border-b border-border">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-muted mt-0.5">اطلاعات زیر را تکمیل کنید تا حسابتون ساخته
                                            بشه</p>
                                    </div>
                                </div>
                                ...
                                <fieldset class="space-y-4">
                                    <legend class="flex items-center gap-2 font-bold text-sm text-foreground mb-1">
                                        <span
                                            class="flex items-center justify-center w-5 h-5 rounded-md bg-primary/10 text-primary text-[10px] font-black border border-primary/20">۱</span>
                                        اطلاعات شخصی
                                    </legend>
                                    ...
                                    <div class="relative" data-tour="codeMell">
                                        <label class="block text-xs font-semibold mb-1.5 text-muted">کد ملی</label>
                                        <input wire:model.blur="codeMell" type="text" maxlength="10"
                                               placeholder="۱۰ رقم" inputmode="numeric" dir="ltr" autocomplete="off"
                                ...
                                <fieldset class="space-y-4 pt-5 border-border">
                                    <legend class="flex items-center gap-2 font-bold text-sm text-foreground mb-1">
                                        <span
                                            class="flex items-center justify-center w-5 h-5 rounded-md bg-primary/10 text-primary text-[10px] font-black border border-primary/20">۲</span>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        @if($grade === 'graduate')
                                            <div
                                                class="col-span-2 flex items-start gap-2 rounded-xl bg-primary/5 border border-primary/20 px-3.5 py-3">
                                                <svg class="w-4 h-4 text-primary shrink-0 mt-0.5" viewBox="0 0 24 24"
                                                     fill="none" stroke="currentColor" stroke-width="2"
                                                     stroke-linecap="round" stroke-linejoin="round">
                                                    <circle cx="12" cy="12" r="10"/>
                                ...
                                <fieldset class="space-y-4 pt-5 border-border">
                                    <legend class="flex items-center gap-2 font-bold text-sm text-foreground mb-1">
                                        <span
                                            class="flex items-center justify-center w-5 h-5 rounded-md bg-primary/10 text-primary text-[10px] font-black border border-primary/20">۳</span>
                                ...
                                <div class="flex items-center justify-between gap-3 pt-5 border-border">
                                    <p class="text-[11px] text-muted leading-5 max-w-[50%]">با ارسال این فرم، یک کد
                                        تأیید روی شماره‌ی موبایلتون ارسال می‌شه.</p>
                                    <button type="submit" @mousedown="pressBtn($el)" :disabled="busy"
                                        ...
                                        <span x-show="busy" class="flex items-center gap-2">
                                            <svg class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle
                                                    cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3"
                                                    stroke-dasharray="32" stroke-linecap="round" opacity="0.5"/></svg>
                                            صبر کنید…
                                        </span>
                                    </button>
                                </div>
                    ...
                    init() {
                        ...
                        if (window.Livewire) {
                            try {
                                ...
                                Livewire.on('step-validation-failed', () => {
                                    this.clearBusy();
                                });
                                ...
                                console.warn('[onboarding] Livewire hook setup failed', e);
                            }
                        }
