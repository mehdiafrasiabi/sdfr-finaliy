<div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100"
    ...
    <div class="max-w-2xl mx-auto px-4 py-6 md:py-10" x-cloak>
        ...
        @if ($generalError)
            <div
                class="mb-4 rounded-lg bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-200 px-4 py-3 border border-rose-200 dark:border-rose-800">
                {{ $generalError }}
            </div>
        @endif
        ...
        <form @submit.prevent="goNext()"
            ...
            <section x-show="$wire.currentStep === 1"
                ...
                <div class="text-center py-6">
                    <div
                        class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 text-3xl mb-4">
                        👋
                    </div>
                    <h2 class="text-2xl font-extrabold mb-2">به SDFR خوش آمدید</h2>
                    <p class="text-slate-600 dark:text-slate-400 leading-relaxed">
                        با چند مرحلهٔ ساده، حساب کاربری شما ساخته می‌شود و مسیر آموزشی شخصی‌سازی‌شدهٔ خود را آغاز
                        می‌کنید.
                    </p>
                </div>
            </section>
            ...
            <section x-show="$wire.currentStep === 2"
                ...
                <div class="text-center py-6">
                    <div
                        class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-3xl mb-4">
                        🎯
                    </div>
            ...
            <section x-show="$wire.currentStep === 3"
                ...
                <div class="text-center py-6">
                    <div
                        class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 text-3xl mb-4">
                        🚀
                    </div>
            ...
            <section x-show="$wire.currentStep === 4"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        @error('firstName')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">نام خانوادگی <span
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold mb-1">کد ملی <span
                                class="text-rose-500">*</span></label>
                        <input wire:model.blur="codeMell" type="text" maxlength="10" placeholder="۱۰ رقم"
                               autocomplete="off" inputmode="numeric"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('codeMell') border-rose-500 @enderror"
                               dir="ltr">
                        @error('codeMell')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input wire:model.blur="fatherMobile" x-model="pf" type="tel" placeholder="09..."
                               autocomplete="off" inputmode="numeric"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('fatherMobile') border-rose-500 @enderror"
                               dir="ltr">
                        @error('fatherMobile')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        ...
                        <input wire:model.blur="motherMobile" x-model="pm" type="tel" placeholder="09..."
                               autocomplete="off" inputmode="numeric"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('motherMobile') border-rose-500 @enderror"
                               dir="ltr">
                        @error('motherMobile')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-1">پایه <span
                                class="text-rose-500">*</span></label>
                    ...
                    @if($grade !== '9')
                        <div>
                            <label class="block text-sm font-semibold mb-1">رشته <span
                                    class="text-rose-500">*</span></label>
            ...
            <section x-show="$wire.currentStep === 6"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold mb-1">استان <span
                                class="text-rose-500">*</span></label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold mb-1">شمارهٔ موبایل (برای ورود) <span
                                class="text-rose-500">*</span></label>
                        <input wire:model.blur="mobile" type="tel" placeholder="09..."
                               autocomplete="off" inputmode="numeric"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('mobile') border-rose-500 @enderror"
                               dir="ltr">
                        @error('mobile')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">رمز عبور <span
                                class="text-rose-500">*</span></label>
                        <input wire:model.blur="password" type="password"
                               autocomplete="new-password" data-lpignore="true" data-1p-ignore="true"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('password') border-rose-500 @enderror"
                               dir="ltr">
                        @error('password')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">تکرار رمز <span
                                class="text-rose-500">*</span></label>
                        <input wire:model.blur="passwordConf" type="password"
                               autocomplete="new-password" data-lpignore="true" data-1p-ignore="true"
                               class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('passwordConf') border-rose-500 @enderror"
                               dir="ltr">
                        @error('passwordConf')
                        <div class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</div>@enderror
                    </div>
            ...
            <section x-show="$wire.currentStep === 7"
                ...
                <input wire:model="otpInput" type="text" maxlength="6" placeholder="------"
                       inputmode="numeric"
                       class="w-full text-center tracking-[0.5em] text-2xl font-mono rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                       dir="ltr">
            ...
            <section x-show="$wire.currentStep === 8"
                ...
                <div class="text-center py-4">
                    <div
                        class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 text-2xl mb-3">
                        ✓
                    </div>
                    <h2 class="text-2xl font-extrabold mb-2">حساب شما ساخته شد</h2>
                    <p class="text-slate-600 dark:text-slate-400 mb-6">برای ادامه یکی از گزینه‌های زیر را انتخاب
                        کنید.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <button type="button" wire:click="goToPurchase"
                        ...
                        <div class="text-xs text-slate-600 dark:text-slate-400">دسترسی به همهٔ امکانات با قیمت پلکانی
                        </div>
                    </button>
                </div>
        ...
        <div x-show="$wire.showTrialConfirm"
            ...
            <div
                class="relative w-full max-w-sm rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl p-6 text-center">
                <div
                    class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 text-2xl mb-3">
                    ✨
                </div>
    ...
    @push('script')
        <script>
            // قبل از اینکه Alpine شروع بشه باید ثبت بشه
            document.addEventListener('alpine:init', () => {
                Alpine.data('onboardingFlow', (initial) => ({
                    ...
                    goNext() {
                        ...
                        if (this.$wire.currentStep >= 1 && this.$wire.currentStep <= 3) {
                            this.$wire.set('currentStep', this.$wire.currentStep + 1);
                            return;
                        }
