<div
    x-data="{
        justPicked: false,
        selectNow() { this.justPicked = true; setTimeout(() => this.justPicked = false, 350); }
    }"
    ...
    <div class="relative z-10 max-w-2xl mx-auto px-4 py-8 pb-32">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:rgba(59,130,246,0.15);border:1px solid rgba(59,130,246,0.3);">
                    <svg class="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2"/></svg>
                </div>
            ...
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-full" style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);">
                ...
                <span class="text-xs font-bold text-blue-300 tabular-nums">{{ $answered }} / {{ $totalActive }}</span>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-4 mb-5" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);">
            ...
            <div class="h-2 rounded-full overflow-hidden" style="background:rgba(255,255,255,0.06);">
                <div class="h-full rounded-full transition-all duration-500" style="background:linear-gradient(to left,#3b82f6,#8b5cf6);width:{{ $percent }}%;"></div>
            </div>
        </div>
            ...
            <div class="rounded-2xl overflow-hidden transition-all duration-250"
                ...
                <div class="p-4 space-y-2">
                    ...
                    <div class="flex items-center gap-2 px-4 py-2.5 rounded-xl mb-3" style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.25);">
                        <svg class="w-4 h-4 text-red-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/></svg>
                    ...
                    @if($question->type === \App\Models\AssessmentQuestion::TYPE_LIKERT5)
                        @foreach($question->options as $opt)
                            <button type="button"
                                ...
                                <span class="text-sm font-medium flex-1 text-right {{ $likertValue === (string)$opt->value ? 'text-white' : 'text-white/60' }}">{{ $opt->label_fa }}</span>
                            </button>
                        @endforeach
                    ...
                    @elseif($question->type === \App\Models\AssessmentQuestion::TYPE_VARK_MULTI)
                        ...
                        @foreach($question->options as $opt)
                            <label class="flex items-center gap-3 px-4 py-3.5 rounded-xl cursor-pointer transition-all duration-200 hover:scale-[1.005]"
                                ...
                                <span class="text-sm font-medium flex-1 text-right {{ in_array($opt->id, $selectedOptionIds) ? 'text-white' : 'text-white/60' }}">{{ $opt->label_fa }}</span>
                            </label>
                        @endforeach
                    ...
                    @else
                        @foreach($question->options as $opt)
                            <button type="button"
                                ...
                                <span class="text-sm font-medium flex-1 text-right {{ (int)$selectedOptionId === $opt->id ? 'text-white' : 'text-white/60' }}">{{ $opt->label_fa }}</span>
                            </button>
                        @endforeach
                    @endif
                </div>
    ...
    @if($question)
        <div class="fixed bottom-0 inset-x-0 z-50" style="background:rgba(10,10,15,0.95);backdrop-filter:blur(24px);border-top:1px solid rgba(255,255,255,0.07);">
            <div class="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
                <div class="text-xs text-white/25 font-mono">{{ $currentIndex }}/{{ $totalActive }}</div>
                <button type="button"
                        wire:click="submitAnswer"
                        wire:loading.attr="disabled"
                        wire:target="submitAnswer"
                        class="inline-flex items-center gap-2 px-7 py-2.5 rounded-xl text-sm font-bold text-white transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]"
