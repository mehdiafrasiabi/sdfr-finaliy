                    ...
                    <div class="border border-border rounded-xl overflow-hidden">
                        <div class="bg-secondary px-4 py-3 border-b border-border">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <h3 class="font-bold text-sm text-foreground">{{ $ticket->title }}</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-4 max-h-[500px] overflow-y-auto px-1" id="messages-container">
                            @foreach($ticket->messages as $msg)
                                @if($msg->user_id === \Illuminate\Support\Facades\Auth::id())
                                    <!-- user message -->
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1 max-w-lg">
                                            <div class="flex items-center gap-2 mb-1">
                                                <span class="font-bold text-xs text-foreground">{{ $msg->user->name }}</span>
                                                <span class="w-1 h-1 bg-border rounded-full"></span>
                                                <span class="text-[10px] text-muted">{{ \Date::parse($msg->created_at)->diffForHumans() }}</span>
                                            </div>
                                            <div class="bg-secondary rounded-xl rounded-tr-sm font-semibold text-xs leading-6 text-muted px-4 py-2.5">
                                                {{ $msg->message }}
                                            </div>
                                            @if($msg->attachment)
                                    ...
                                    <div class="flex items-start gap-3 flex-row-reverse">
                                        ...
                                        <div class="flex-1 max-w-lg">
                                            <div class="flex items-center gap-2 mb-1 justify-end">
                                                <span class="text-[10px] text-muted">{{ \Date::parse($msg->created_at)->diffForHumans() }}</span>
                                                <span class="w-1 h-1 bg-border rounded-full"></span>
                                                <span class="font-bold text-xs text-blue-500">{{ $msg->admin->name ?? 'پشتیبان' }}</span>
                                            </div>
                                            <div class="bg-blue-500 text-white rounded-xl rounded-tl-sm font-semibold text-xs leading-6 px-4 py-2.5">
                                                {{ $msg->message }}
                                            </div>
                                            @if($msg->attachment)
                                                <div class="text-left">
                    ...
                    <div class="border border-border rounded-xl overflow-hidden">
                        ...
                        @else
                            <div class="flex flex-col items-center justify-center space-y-4 py-8 px-4 bg-secondary">
                                ...
                                <p class="font-bold text-sm text-foreground text-center">
