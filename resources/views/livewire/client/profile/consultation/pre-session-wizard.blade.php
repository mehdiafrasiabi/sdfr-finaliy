                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-col items-stretch gap-2 sm:items-end">
                        <a wire:navigate href="{{ route('client.profile.consultation.sessions') }}"
                           class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-red-500 px-4 py-2 text-xs sm:text-sm font-medium text-white shadow-sm transition hover:bg-red-600">
                        ...
                        <span class="inline-flex items-center gap-2 rounded-full bg-black/10 px-3 py-1 text-[11px] text-blue-100/90 ring-1 ring-blue-200/40">
                            ...
                            <span>مرحله فعلی: <strong class="mr-1">{{ $stepTitles[$currentStep] ?? '' }}</strong></span>
                        </span>
                    </div>
                </div>
            </div>
                    ...
                    @foreach($stepTitles as $step => $title)
                        <div class="flex items-center {{ $step < $totalSteps ? 'flex-1' : '' }}">
                            <button wire:click="goToStep({{ $step }})"
                                ...
                                @if($currentStep === $step) bg-blue-600 text-white shadow-sm ring-2 ring-blue-300/80
                                @elseif($currentStep > $step) bg-emerald-500 text-white shadow-sm ring-1 ring-emerald-300/70
                                @else bg-muted text-muted-foreground ring-1 ring-border @endif">
                                @if($currentStep > $step)
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
                                @else
                                    {{ $step }}
                                @endif
                            </button>
            ...
            <div class="bg-card p-4 sm:p-6 border-t border-border rounded-b-2xl">
                ...
                @if(!$canEdit)
                    <div class="mb-6 flex items-start gap-2 rounded-xl border border-amber-200/80 bg-amber-50 dark:bg-amber-500/10 px-3 py-3 text-xs text-amber-700 dark:border-amber-500/40 dark:text-amber-300">
                        ...
                        <p>زمان ویرایش پیش‌جلسه به پایان رسیده است. <span class="font-medium">فقط می‌توانید اطلاعات ثبت شده را مشاهده کنید.</span></p>
                    </div>
                @endif
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 mb-4">
                                ...
                                <div>
                                    ...
                                    @if(count($availableSubjects) > 0)
                                        ...
                                        />
                                    @else
                                        <input type="text" wire:model="examForm.subject"
                                               class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                                ...
                                @include('livewire.client.profile.consultation.partials.time-picker', ['wireModel' => 'examForm.time_per_part', 'xDataKey' => 'examTimePicker'])
                            </div>
                        ...
                        <div class="space-y-2">
                            @foreach($exams as $exam)
                                <div class="flex items-center justify-between rounded-xl bg-blue-500/10 dark:bg-blue-500/15 px-3 py-2.5 text-xs sm:text-sm text-foreground">
                                    <div>
                                        <span class="text-muted-foreground">امتحان ({{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($exam['exam_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}) :</span>
                                    ...
                                    @if($canEdit)
                                        <button wire:click="deleteExam({{ $exam['id'] }})" class="text-red-500 transition hover:text-red-600 mr-2 shrink-0">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M6 6l1 16h10l1-16"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                                        </button>
                                    @endif
                                </div>
                            @endforeach
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 mb-4">
                                ...
                                <div>
                                    ...
                                    @if(count($availableSubjects) > 0)
                                        ...
                                        />
                                    @else
                                        <input type="text" wire:model="qaForm.subject"
                                               class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                                ...
                                @include('livewire.client.profile.consultation.partials.time-picker', ['wireModel' => 'qaForm.time_per_part', 'xDataKey' => 'qaTimePicker'])
                            </div>
                            <div class="flex justify-end gap-3 pt-4">
                        ...
                        <div class="space-y-2">
                            @foreach($qas as $qa)
                                <div class="flex items-center justify-between rounded-xl bg-emerald-500/10 dark:bg-emerald-500/15 px-3 py-2.5 text-xs sm:text-sm text-foreground">
                                    <div>
                                        <span class="text-muted-foreground">پرسش و پاسخ ({{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($qa['qa_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}) :</span>
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 mb-4">
                                ...
                                <div>
                                    ...
                                    @if(count($availableSubjects) > 0)
                                        ...
                                        />
                                    @else
                                        <input type="text" wire:model="assignmentForm.subject"
                                               class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                                ...
                                @include('livewire.client.profile.consultation.partials.time-picker', ['wireModel' => 'assignmentForm.time_per_part', 'xDataKey' => 'assignmentTimePicker'])
                            </div>
                            <div class="flex justify-end gap-3 pt-4">
                        ...
                        <div class="space-y-2">
                            @foreach($assignments as $assignment)
                                <div class="flex items-center justify-between rounded-xl bg-violet-500/10 dark:bg-violet-500/15 px-3 py-2.5 text-xs sm:text-sm text-foreground">
                                    <div>
                                        <span class="text-muted-foreground">تکلیف ({{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($assignment['due_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}) :</span>
                ...
                {{-- ===== مرحله ۴: پارت در خواستی ===== --}}
                @if($currentStep === 4)
                    <h3 class="mb-2 text-base sm:text-lg font-bold text-foreground">پارت در خواستی</h3>
                    <p class="mb-5 text-xs sm:text-sm text-muted-foreground">درس‌هایی که می‌خواهید در برنامه هفتگی برای شما لحاظ شود را مشخص کنید.</p>
                    ...
                    @if($canEdit)
                        <div class="mb-6 rounded-xl bg-muted/50 dark:bg-muted/30 px-3 py-4 sm:px-4 sm:py-5">
                            <div class="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 mb-4">
                                ...
                                <div>
                                    <label class="mb-1 block text-xs font-medium text-foreground">
                                        فصل <span class="text-muted-foreground font-normal">(اختیاری)</span>
                                    </label>
                                    <x-ui.select
                                        wire:model="requestedPartForm.cc_chapter_id"
                                        :options="$requestedPartChapters"
                                        value-key="id"
                                        label-key="name"
                                        placeholder="ابتدا درس را انتخاب کنید"
                                ...
                                <div class="md:col-span-2">
                                    <label class="mb-1 block text-xs font-medium text-foreground">
                                        توضیحات <span class="text-muted-foreground font-normal">(اختیاری)</span>
                                    </label>
                                    <textarea wire:model="requestedPartForm.description" rows="4"
                                              class="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-4 py-3 mt-1 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition"
                                ...
                                @include('livewire.client.profile.consultation.partials.time-picker', ['wireModel' => 'requestedPartForm.time_per_part', 'xDataKey' => 'requestedPartTimePicker'])
                            </div>
                            <div class="flex justify-end gap-3 pt-4">
                        ...
                        <div class="space-y-2">
                            @foreach($requestedParts as $rp)
                                <div class="flex items-start justify-between rounded-xl bg-orange-500/10 dark:bg-orange-500/15 px-3 py-2.5 text-xs sm:text-sm text-foreground">
                                    ...
                                    @if($canEdit)
                                        <button wire:click="deleteRequestedPart({{ $rp['id'] }})" class="text-red-500 transition hover:text-red-600 mr-2 shrink-0 mt-0.5">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M6 6l1 16h10l1-16"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                                        </button>
                                    @endif
                                </div>
                ...
                @if($currentStep === 5)
                    ...
                    <p class="mb-5 text-xs sm:text-sm text-muted-foreground">هر توضیح یا نکته دیگری که می‌خواهید به مشاور بگویید را اینجا بنویسید.</p>

                    <div class="mb-4">
                        <textarea wire:model="miscDescription" rows="6"
                                  class="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-3 text-sm shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 {{ !$canEdit ? 'opacity-60 cursor-not-allowed' : '' }}"
                ...
                @if($currentStep === 6)
                    ...
                    <div class="space-y-4 sm:space-y-6">
                        <div class="rounded-xl border border-border bg-muted/30 dark:bg-muted/20 p-3 sm:p-4">
                            ...
                            @forelse($exams as $exam)
                                <div class="border-b border-dashed border-border py-1.5 text-xs sm:text-sm last:border-b-0 text-foreground">
                                    {{ $exam['subject'] }} – {{ $exam['part_count'] }} پارت ({{ $exam['time_per_part'] }} دقیقه) – {{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($exam['exam_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}
                                </div>
                            @empty
                        ...
                        <div class="rounded-xl border border-border bg-muted/30 dark:bg-muted/20 p-3 sm:p-4">
                            ...
                            @forelse($qas as $qa)
                                <div class="border-b border-dashed border-border py-1.5 text-xs sm:text-sm last:border-b-0 text-foreground">
                                    {{ $qa['subject'] }} – {{ $qa['part_count'] }} پارت ({{ $qa['time_per_part'] }} دقیقه) – {{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($qa['qa_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}
                                </div>
                            @empty
                                <p class="text-xs text-muted-foreground">ثبت نشده</p>
                            @endforelse
                        ...
                        <div class="rounded-xl border border-border bg-muted/30 dark:bg-muted/20 p-3 sm:p-4">
                            ...
                            @forelse($assignments as $assignment)
                                <div class="border-b border-dashed border-border py-1.5 text-xs sm:text-sm last:border-b-0 text-foreground">
                                    {{ $assignment['subject'] }} – {{ $assignment['part_count'] }} پارت ({{ $assignment['time_per_part'] }} دقیقه) – {{ \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($assignment['due_date'], 'UTC')->setTimezone('Asia/Tehran'))->format('Y/m/d') }}
                                </div>
                            @empty
                                <p class="text-xs text-muted-foreground">ثبت نشده</p>
                            @endforelse
                    ...
                    <div>
                        @if($currentStep > 1)
                            <button wire:click="prevStep"
                                    class="inline-flex items-center justify-center rounded-lg bg-orange-400 px-5 py-2 text-xs sm:text-sm font-medium text-white shadow-sm transition hover:bg-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-300">
                                مرحله قبل
                            </button>
                        @endif
                    </div>
