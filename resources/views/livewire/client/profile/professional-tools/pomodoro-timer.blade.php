<div class="max-w-7xl space-y-14 px-4 mx-auto">
        ...
        <div class="lg:col-span-9 md:col-span-8">
            <div class="space-y-10">
                <div class="space-y-5">
                    ...
                    {{-- Main Card --}}
                    <div class="bg-secondary dark:bg-secondary  text-white rounded-2xl shadow-2xl p-6 border border-white/10">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full">
                                <div class="w-3 h-3 rounded-full animate-pulse {{ $isRunning ? 'bg-green-400' : 'bg-red-400' }}"></div>
                        ...
                        <div class="mt-6 bg-black/30 rounded-2xl p-6 shadow-inner">
                            {{-- Digital Clock via Tailwind font + tracking --}}
                            <div class="flex justify-center leading-none font-mono text-7xl tracking-widest text-amber-400 [text-shadow:0_0_8px_rgba(255,150,0,0.7)]" dir="ltr"
                                 style="font-family:'Digital','Courier New',monospace">
                            ...
                            <div class="mt-4 h-3 bg-white/10 rounded-full overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-amber-400 via-lime-400 to-emerald-400 transition-all duration-1000"
                                     style="width: {{ $progress }}%"></div>
                            </div>
                            <p class="text-center text-xs mt-2 text-slate-300">پیشرفت این مرحله بر اساس قوانین پومودرو</p>
                        </div>

                        {{-- Controls --}}
                        <div class="mt-6 grid md:grid-cols-2 gap-3">
                            <button wire:click="{{ $isRunning ? 'stop' : 'start' }}"
                                    class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-bold transition
                                        {{ $isRunning
                            ...
                            <button wire:click="resetTimer"
                                    class="w-full px-4 py-3 rounded-xl font-bold transition bg-yellow-500 hover:bg-yellow-400 text-slate-900">
                                شروع تازه چرخه
                            </button>

                            <button wire:click="skipPhase"
                                    class="w-full px-4 py-3 rounded-xl font-bold transition bg-lime-950 hover:bg-sky-600 text-white">
                                پرش به مرحله بعد (بدون ثبت)
                            </button>

                            <label class="w-full px-4 py-3 rounded-xl font-bold transition bg-white/5 hover:bg-white/10 flex items-center gap-3 cursor-pointer">
                        ...
                        {{-- Stats --}}
                        <div class="mb-5 mt-6 grid md:grid-cols-2 gap-4 text-sm">
                        ...
                        {{-- Rules --}}
                        <div class="mb-5 p-4 rounded-xl bg-gradient-to-r from-white/5 via-white/10 to-white/5 border border-white/5 text-xs leading-6 text-slate-200">
            ...
            <div class="p-5 text-xs text-muted whitespace-nowrap text-white"></div>
        </div>
    </div>
    ...
    <script>
        ...
        window.addEventListener('study-saved-success', () => startConfetti());
